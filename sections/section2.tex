...
\begin{theo}[性质]
\begin{itemize}
\item 若 $x$ 取遍模 $m$ 的完全剩余系，则 $ax+b$ 也取遍模 $m$ 的完全剩余系。
\item 若 $\gcd(a,m)=1$，$x$ 取遍模 $m$ 的简化剩余系，则 $ax$ 也取遍模 $m$ 的简化剩余系。
\item 若 $x_1,x_2$ 分别取遍模 $m_1,m_2$ 的完全剩余系，则 $m_2x_1+m_1x_2$ 取遍模 $m_1m_2$ 的完全剩余系。
...
\begin{frame}
其中 $u_0$ 只与 $P,Q$ 有关。将 $A_r$ 中的每个元素 $a$ 变为 $(g-Pu_0)\dfrac{a-r}{g}=Qv_0\dfrac{a-r}{g}$，$B_r$ 中的每个元素 $b$ 变为 $Pu_0\dfrac{b-r}{g}$。问题变成求有多少 $x$ 满足 $\exists a,b,\ \mathrm{s.t.}\ x\equiv a+b+r\pmod{\mathrm{lcm}(P,Q)}$。
...
\begin{theo}[简单性质]
\begin{itemize}
\item $\left(\dfrac{a}{p}\right)\equiv a^{\frac{p-1}{2}}\pmod p$
\item $\left(\dfrac{ab}{p}\right)=\left(\dfrac{a}{p}\right)\left(\dfrac{b}{p} \right)$（完全积性）
...
\subsubsection{P6610 [Code+\#7] 同余方程}
