...
\begin{frame}{整除}
我们知道任意两个整数的和、差、积以至任意有限个整数的加、减、乘的混合运算结果仍是整数，但是整数除整数不一定仍为整数。\pause
...
\begin{frame}{取整函数}
对于任意 $x\in \R$，用 $\left[x\right]$ 或 $\lfloor x \rfloor$ 表示不大于 $x$ 的最大整数。用 $\lceil x\rceil$ 表示不小于 $x$ 的最小整数。用 $\left\{x\right\}=x-\lfloor x\rfloor$ 表示 $x$ 的小数部分。 \pause

\bigskip
下面是几条性质：\pause

\begin{itemize}
\item $\lfloor x\rfloor\le x<\lfloor x\rfloor+1$，$x-1<\lfloor x\rfloor\le x$\pause
\item $\lfloor n+x\rfloor=n+\lfloor x\rfloor$，$n$ 为整数\pause
\item $\lfloor x\rfloor+\lfloor y\rfloor\le\lfloor x+y\rfloor$\pause
...
\begin{frame}{最大公因数}
\begin{defi}[gcd \& lcm]
$a,b\in \Z$，若 $d\mid a,d\mid b$，则称 $d$ 为 $a,b$ 的公因数。\pause

当 $a,b$ 不全为 $0$ 时公因数个数是有限的，故存在最大公因数，记为 $\gcd(a,b)$ 或 $(a,b)$。 \pause

特别地，若 $a=b=0$，定义 $\gcd(0,0)=0$。 \pause
...
\begin{frame}
\begin{itemize}
\item $\gcd(a,b)=\gcd(|a|,|b|)$ \pause
\item $\gcd(\gcd(a,b),c)=\gcd(a,\gcd(b,c))$ \pause
\item $\mathrm{lcm}(\mathrm{lcm}(a,b),c)=\mathrm{lcm}(a,\mathrm{lcm}(b,c))$ \pause
\item $\gcd(a,b)\mathrm{lcm}(a,b)=ab$ \pause
\item $d\mid a,d\mid b\iff d\mid\gcd(a,b)$ \pause
\item $\gcd(ma,mb)=m\gcd(a,b)$ \pause
...
\begin{frame}
每次操作后 $ab$ 至少会变为原来的 $1/2$，故时间复杂度 $O(\log a+\log b)$。 \pause

精细的分析是 $O(\log\min\{a,b\})$。 \pause
...
对指数使用辗转相除法，可证 $\gcd(a^n-1,a^m-1)=a^{\gcd(n,m)}-1$。 \pause
...
对于方程 $ax+by=c$，其有解的充要条件是 $\gcd(a,b)\mid c$。 \pause

调用 exgcd，得到的结果记为 $x_0,y_0$。 \pause
\medskip

一组特解为 $x_1=\dfrac{x_0c}{\gcd(a,b)},y_1=\dfrac{y_0c}{\gcd(a,b)}$。 \pause
...
\begin{frame}{P3518 [POI 2011] SEJ-Strongbox}
...
\begin{frame}
设最小非零的密码为 $d$，则所有 $d$ 的倍数也是密码。若存在密码 $x$ 不是 $d$ 的倍数，则由裴蜀定理 $\gcd(x,d)<d$ 也是密码，与 $d$ 的最小性矛盾。故所有密码恰是 $d$ 的所有倍数。进一步可知 $d\mid n$。 \pause
...
$a,b$ 互素的充要条件是 $\exists u,v\in \Z,ua+vb=1$。 \pause

\begin{theo}
\begin{itemize}
\item 如果 $a\mid bc$ 且 $(a,b)=1$，则 $a\mid c$ \pause
\item 如果 $a\mid c,b\mid c$ 且 $(a,b)=1$，则 $ab\mid c$ \pause
...
在一般的代数系统中，素不等同于不可约。$\Z$ 中二者恰好等价，是由于 $\Z$ 本身有良好的结构。 \pause
...
同样的，一般的代数系统中不一定存在这种唯一分解定理。 \pause

分解质因数是数论问题的重要思考方向。将素数从小到大排列为 $p_1=2,p_2=3,p_3=5,\dots$，则任一正整数 $n=p_1^{\alpha_1}p_2^{\alpha_2}\dots$ 可映射为一个每维都是非负整数的无穷维向量 $(\alpha_1,\alpha_2,\dots)$。

正整数的乘法与向量的加法形成同构，$\gcd$ 对应于每维分别取 $\min$，$\mathrm{lcm}$ 对应于取 $\max$。
\end{frame}

\subsubsection{}

\begin{frame}{P8255 [NOI Online 2022 入门组] 数学游戏}
...
\begin{frame}
对于每个 $p$，记 $v_p(x)$ 为 $x$ 的分解式中 $p$ 上的指数。则可知有解的必要条件是 $\forall p\in \mathbb{P},v_p(x)\le v_p(z)$，并且若 $3v_p(x)\le v_p(z)$，则 $v_p(y)=v_p(z)-2v_p(x)$，否则 $2v_p(y)=v_p(z)-v_p(x)$。 \pause
