...
\begin{frame}[fragile,allowframebreaks]{互素}
\begin{itemize}
    \item 互素定义
...
\begin{frame}[fragile,allowframebreaks]{素数}
...
\begin{frame}[fragile,allowframebreaks]{P8255 [NOI Online 2022 入门组] 数学游戏}
...
\begin{frame}[fragile,allowframebreaks]{单个方程}
...
\begin{frame}[fragile,allowframebreaks]{方程组}
求解线性同余方程组：
$$
\left\{
\begin{aligned}
...
\begin{frame}[fragile,allowframebreaks]{中国剩余定理}
...
\begin{frame}[fragile,allowframebreaks]{CF338D GCD Table}
...
\begin{frame}[fragile,allowframebreaks]{P5330 [SNOI2019] 数论}
...
其中 $u_0$ 只与 $P,Q$ 有关。将 $A_r$ 中的每个元素 $a$ 变为 $(g-Pu_0)\dfrac{a-r}{g}=Qv_0\dfrac{a-r}{g}$，$B_r$ 中的每个元素 $b$ 变为 $Pu_0\dfrac{b-r}{g}$。问题变成求有多少 $x$ 满足 $\exists a,b,\ \mathrm{s.t.}\ x\equiv a+b+r\pmod{\mathrm{lcm}(P,Q)}$。注意到不会有两对不同的 $a,b$ 撞在一起，于是容易解决。
\end{frame}

\begin{frame}[fragile,allowframebreaks]{素数模的同余式}
...
\begin{frame}[fragile,allowframebreaks]{二次剩余}
...
\begin{frame}[fragile,allowframebreaks]{欧拉判别法}
...
\begin{frame}[fragile,allowframebreaks]{勒让德符号}
...
\begin{frame}[fragile,allowframebreaks]{Cipolla 算法}
...
\begin{frame}[fragile,allowframebreaks]{P6610 [Code+\#7] 同余方程}
...
\begin{frame}[fragile,allowframebreaks]{卢卡斯定理}
\begin{itemize}
    \item 卢卡斯定理
...
\begin{frame}[fragile,allowframebreaks]{P5598 混乱度}
...
\begin{frame}[fragile,allowframebreaks]{P4345 [SHOI2015] 超能粒子炮·改}
...
\begin{frame}[fragile,allowframebreaks]{扩展卢卡斯定理}
...
\begin{frame}[fragile,allowframebreaks]{勒让德定理}
\begin{itemize}
    \item 勒让德定理
...
\begin{frame}[fragile,allowframebreaks]{库默尔定理}
\begin{itemize}
    \item 库默尔定理
...
\begin{frame}[fragile,allowframebreaks]{SOJ2028 梅子}
...
\begin{frame}[fragile,allowframebreaks]{CF582D Number of Binominal Coefficients}
...
\begin{frame}[fragile,allowframebreaks]{阶的定义}
\begin{itemize}
    \item 阶
...
\begin{itemize}
    \item $\delta_m(a^k)=\dfrac{\delta_m(a)}{\gcd(\delta_m(a),k)}$
    \item 若 $\gcd(\delta_m(a),\delta_m(b))=1$，则 $\delta_m(ab)=\delta_m(a)\delta_m(b)$
    \item $\delta_m(a)=\delta_m(a^{-1})$
\end{itemize}
\end{frame}

\begin{frame}[fragile,allowframebreaks]{阶的计算}
...
\begin{frame}[fragile,allowframebreaks]{离散对数判定}
...
\begin{frame}[fragile,allowframebreaks]{ABC335G Discrete Logarithm Problems}
一道算法竞赛中罕见的和阶有关的题（
...
\begin{frame}[fragile,allowframebreaks]{原根的定义}
...
更加广泛的结论是，若 $m$ 存在原根，则使得 $\delta_m(a)=l$ 的 $a$ 的个数为 $\left\{\begin{aligned}&0 &l\nmid\varphi(m)\\&\varphi(l) &l\mid\varphi(m) \end{aligned} \right.$。
\end{frame}

\begin{frame}[fragile,allowframebreaks]{原根的计算}
...
\begin{frame}[fragile,allowframebreaks]{BSGS 算法}
...
\begin{frame}[fragile,allowframebreaks]{高次剩余}
...
\begin{frame}[fragile,allowframebreaks]{UOJ525 平行四边形}
...
\begin{frame}[fragile,allowframebreaks]{CF360D Levko and Sets}
...
\begin{frame}[fragile,allowframebreaks]{定义}
\begin{itemize}
    \item 定义
...
\begin{frame}[fragile,allowframebreaks]{简单运算}
\begin{itemize}
    \item 加法
...
\begin{frame}[fragile,allowframebreaks]{常见积性函数}
\begin{itemize}
    \item $\epsilon(n)=[n=1]$
  ...
  （证明 $\varphi$ 是积性函数）
    ...
    \item $\mu(n)=\begin{cases}1 &n=1\\0 &\exists d>1,d^2\mid n\\(-1)^{\omega(n)} &\text{otherwise} \end{cases}$，其中 $\omega(n)$ 为 $n$ 含有的不同素因子个数

    \item $\chi_k(n)=[\gcd(n,k)=1]$
\end{itemize}
\end{frame}

\begin{frame}[fragile,allowframebreaks]{积性函数线性筛}
...
\begin{frame}[fragile,allowframebreaks]{定义与性质}
...
\begin{frame}[fragile,allowframebreaks]{逆函数}
...
\begin{frame}[fragile,allowframebreaks]{狄利克雷前缀和}
给定一个函数 $f$，求 $f*1$。
...
\begin{frame}[fragile,allowframebreaks]{GYM101741F GCD}
...
\begin{frame}[fragile,allowframebreaks]{积性函数卷积}
\begin{itemize}
...
\begin{frame}[fragile,allowframebreaks]{常见贝尔级数}
下面给出一些常见积性函数的贝尔级数：
...
\begin{frame}[fragile,allowframebreaks]{应用}
\begin{itemize}
    \item 经典恒等式 $\varphi*1=\text{id}$，$\mu*1=\epsilon$
    \item 处理逆元，求一个函数 $f$ 使得 $f*d=\epsilon$
\end{itemize}
...
\begin{frame}[fragile,allowframebreaks]{基本结论}
\begin{itemize}
    \item 结论一
  $$
...
\begin{frame}[fragile,allowframebreaks]{P2260 模积和}
...
\begin{frame}[fragile,allowframebreaks]{ARC068E Snuke Line}
...
\begin{frame}[fragile,allowframebreaks]{线性预处理}
\begin{itemize}
    \item 问题
...
\begin{frame}[fragile,allowframebreaks]{基本形式}
...
\begin{frame}[fragile,allowframebreaks]{gcd 卷积}
...
\begin{frame}[fragile,allowframebreaks]{gcd 求和}
\begin{itemize}
    \item 引例
  $$
...
\begin{frame}[fragile,allowframebreaks]{P2257 YY 的 GCD}
求 $\sum\limits_{i=1}^n\sum\limits_{j=1}^m[\gcd(i,j)\in\P]$。$1\le n,m\le 10^7$。

令 $f(n)=[n\in \P]$，需要求 $g=f*\mu$。

发现若 $x=\prod\limits p_i^{\alpha_i}$ 中有两个 $\alpha_i$ 大于 $1$，则 $g(x)$ 一定为 $0$。简单分析知 $g$ 可以线筛。
\end{frame}

\begin{frame}[fragile,allowframebreaks]{P6156 简单题}
求 $\sum\limits_{i=1}^n\sum\limits_{j=1}^n(i+j)^k\mu^2(\gcd(i,j))\gcd(i,j)$。

$n\le 5\times10^6,k\le 10^{18}$。

沿用之前的方法，记 $f=\mu^2\cdot\text{id},g=f*\mu$。原式化为 $\sum\limits_{i=1}^n\sum\limits_{j=1}^n(i+j)^k\sum\limits_{d\mid i,j}g(d)$，即 $\sum\limits_{d=1}^ng(d)d^k\sum\limits_{i=1}^{\lfloor n/d\rfloor}\sum\limits_{j=1}^{\lfloor n/d\rfloor}(i+j)^k$。后面的二重求和式记作 $S(x)=\sum\limits_{i=1}^x\sum\limits_{j=1}^x(i+j)^k$，容易在线性时间内预处理出 $S$ 在 $1\sim n$ 上每个点的取值。
...
\begin{frame}[fragile,allowframebreaks]{SOJ1985 拼图}
...
\begin{itemize}
  ...
  $M=I$ 时显然可以，$M\neq I$ 时，记 $M=PM_1$，$P$ 为 $L$ 或 $R$。可知 $M\begin{pmatrix}a\\b\end{pmatrix}\le\begin{pmatrix}n\\n\end{pmatrix}$ 等价于 $\begin{pmatrix}1 & 1\end{pmatrix}M_1\begin{pmatrix}a\\b\end{pmatrix}\le n$。于是答案为 $1+2\sum\limits_{M}[\begin{pmatrix}1 & 1\end{pmatrix}M\begin{pmatrix}a\\b\end{pmatrix}\le n]$。
...
\begin{frame}[fragile,allowframebreaks]{块筛}
...
\begin{frame}[fragile,allowframebreaks]{杜教筛}
...
\begin{frame}[fragile,allowframebreaks]{积式}
...
\begin{frame}[fragile,allowframebreaks]{P4318 完全平方数}
...
\begin{frame}[fragile,allowframebreaks]{来源不明题}
...
\begin{frame}[fragile,allowframebreaks]{51nod1220 约数之和}
...
\begin{itemize}
  ...
  在集合 $S=\{(i,j)\mid i\mid x,j\mid y,\gcd(i,j)=1 \}$ 与 $D=\{d\mid xy\}$ 之间构造映射。$(i,j)$ 对应于 $d=\dfrac{x}{i}\cdot j$，$d$ 对应于 $(\dfrac{x}{\gcd(d,x)},\dfrac{d}{\gcd(d,x)})$。
...
\begin{frame}[fragile,allowframebreaks]{P1587 [NOI2016] 循环之美}
...
\begin{frame}[fragile,allowframebreaks]{Powerful Number}
...
\begin{frame}[fragile,allowframebreaks]{素数拟合}
给定一个积性函数 $f$，求 $S_f(n)$。
...
\begin{frame}[fragile,allowframebreaks]{P5325 【模板】Min\_25 筛}
...
\begin{frame}[fragile,allowframebreaks]{LOJ6053 简单的函数}
...
\begin{frame}[fragile,allowframebreaks]{有理数表示}
\begin{itemize}
    \item 一个有理数有且仅有两种连分数表示
...
\begin{frame}[fragile,allowframebreaks]{渐近分数}
...
\begin{frame}[fragile,allowframebreaks]{P10788 [NOI2024] 分数}
