...
对于每个 $p$，记 $v_p(x)$ 为 $x$ 的分解式中 $p$ 上的指数。则可知有解的必要条件是 $\forall p\in \P,v_p(x)\le v_p(z)$，并且若 $3v_p(x)\le v_p(z)$，则 $v_p(y)=v_p(z)-2v_p(x)$，否则 $2v_p(y)=v_p(z)-v_p(x)$。
...
\subsubsection{CF338D GCD Table}
\begin{frame}[fragile,allowframebreaks]
...
\subsubsection{P5330 [SNOI2019] 数论}
\begin{frame}[fragile,allowframebreaks]
...
\subsubsection{ABC335G Discrete Logarithm Problems}
\begin{frame}[fragile,allowframebreaks]
一道算法竞赛中罕见的和阶有关的题（
...
更加广泛的结论是，若 $m$ 存在原根，则使得 $\delta_m(a)=l$ 的 $a$ 的个数为 $\begin{cases}0 &l\nmid\varphi(m)\\\varphi(l) &l\mid\varphi(m) \end{cases}$。
\end{frame}
...
\subsubsection{CF360D Levko and Sets}
\begin{frame}[fragile,allowframebreaks]
...
\subsubsection{ARC068E Snuke Line}
\begin{frame}[fragile,allowframebreaks]
...
\end{aligned}
$$
所有的 $h(p^k)$ 都可以在找 PN 的过程中暴力求出。这部分的复杂度为 $O(\sum\limits_{p\in\P,p\le \sqrt n}\log_p^2(n))=O(\dfrac{\sqrt n}{\log n})$，不为瓶颈。
